...
export default function QqqTrailStudyPage({ data }) {
  if (!data) return <p>Loading…</p>

  const { configs, holdComparison, qqqPrices } = data
  ...
  // Combined chart data
  const dateMap = {}
  ;(qqqPrices || []).forEach(p => { dateMap[p.date] = { date: p.date, qqq: p.close } })
  ...
  return (
    <div>
      <h2>QQQ Overnight — Trail Stop Study</h2>
      <p style={{color:'#b0b0b8', marginBottom:16}}>
        Vanilla = unfiltered baseline. Trail configs add QQQ {'>'} SMA50 + pause filter + trailing stop.
      </p>
      ...
      <div className="card">
        ...
        <table>
          ...
          <tbody>
            ...
            {selected !== 'vanilla' && <tr><td><strong>Regime Filter</strong></td><td>QQQ {'>'} SMA(50) — only trade in uptrends</td></tr>}
      ...
      <div className="card">
        ...
        <ResponsiveContainer width="100%" height={380}>
          <ComposedChart data={combined} margin={{ top: 5, right: 50, bottom: 30, left: 10 }}>
            ...
            <YAxis yAxisId="qqq" orientation="right" tick={{ fill: '#555', fontSize: 10 }} tickFormatter={v => v.toLocaleString()} />
            <Tooltip
              contentStyle={{ background: '#1a1d28', border: '1px solid #2a2d3a', borderRadius: 8 }}
              labelStyle={{ color: '#8e8e9a' }}
              formatter={(v, name) => name === 'QQQ' ? [v?.toLocaleString(), name] : [`$${(v||0).toFixed(0)}`, name]}
            />
            <Legend />
            <Area yAxisId="qqq" type="monotone" dataKey="qqq" name="QQQ" stroke="#555" fill="#2a2d3a" fillOpacity={0.3} dot={false} />
      ...
      <div className="card" style={{borderLeft:'3px solid #f59e0b'}}>
        <h3>📝 QQQ Overnight Learnings</h3>
        <ul style={{lineHeight:2, color:'#d1d1d8'}}>
          <li><strong>Trail Forever dominates</strong> — $12.7K P&L, $844 DD, 15× P&L/DD ratio on QQQ.</li>
          <li>Trail 3d is the safer option — higher win rate (73%) with controlled hold time.</li>
          <li>SMA50 filter + pause after 2 losses eliminates the worst drawdown periods.</li>
          <li>QQQ is more volatile than SPX — slightly lower P&L/DD but still strong edge.</li>
        </ul>
      </div>
    </div>
  )

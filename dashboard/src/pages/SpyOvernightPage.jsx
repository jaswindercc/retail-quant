...
export default function SpyOvernightPage({ data }) {
  if (!data) return <div className="loading">Loading SPY overnight data…</div>
  ...
  return (
    <div>
      <h1 className="page-title">SPY Overnight <span>Same model as SPX · Uses SPY ETF data · Enter at close · Exit next day close</span></h1>

      <div className="card strategy-summary">
        <h3>How It Works</h3>
        <ul>
          <li><strong>Same scoring model as SPX Overnight</strong> — applied to SPY ETF instead of SPX index</li>
          ...
          <li><strong>Risk:</strong> $100 per trade. SPY is tradeable (unlike SPX index directly).</li>
        </ul>

        <h3>SPY vs SPX</h3>
        <ul>
          <li><strong>SPX</strong> = S&P 500 index (not directly tradeable, used for options/futures)</li>
          <li><strong>SPY</strong> = ETF that tracks S&P 500 (tradeable, has volume, can short)</li>
          <li>Both follow same signals — SPY is what you'd actually trade in a real account</li>
        </ul>
      </div>
      ...
      <>
        ...
        <div className="card">
          ...
          <p style={{ color: '#8e8e9a', fontSize: 13, marginBottom: 12 }}>
            Same signals as SPX Overnight. Bullish signals add to the score (go long), bearish signals subtract (go short). We only trade when total score ≥ +3 (long) or ≤ −3 (short).
          </p>
          <table>
            <thead>

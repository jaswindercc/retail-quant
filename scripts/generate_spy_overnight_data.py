#!/usr/bin/env python3
"""
SPY OVERNIGHT Strategy — Same model as SPX Overnight but using SPY ETF data.
Uses SPY + VIX data to predict next-day close direction.
"""
...
OUT = Path("/workspaces/jas/dashboard/public/spy_overnight_data.json")
RISK = 100.0
...
spy_df = load(DATA_DIR / "spy_data.csv")
vix_df = load(DATA_DIR / "VIX_daily_data_right - Sheet1.csv")
print(f"  SPY: {len(spy_df)} bars ({spy_df['Date'].min().strftime('%Y-%m-%d')} to {spy_df['Date'].max().strftime('%Y-%m-%d')})")
print(f"  VIX: {len(vix_df)} bars ({vix_df['Date'].min().strftime('%Y-%m-%d')} to {vix_df['Date'].max().strftime('%Y-%m-%d')})")

print("\nRunning SPY overnight prediction backtest...")

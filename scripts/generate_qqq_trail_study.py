#!/usr/bin/env python3
"""
QQQ OVERNIGHT — Trailing Stop Study
======================================
Same methodology as the SPX trail study but applied to QQQ.
...
OUT = Path("/workspaces/jas/dashboard/public/qqq_trail_study.json")
RISK = 100.0
...
def main():
    ...
    spy_df = load(DATA_DIR / "qqq_daily_data_right - Sheet1.csv")
    vix_df = load(DATA_DIR / "VIX_daily_data_right - Sheet1.csv")
    print(f"  QQQ: {len(spy_df)} bars")
    ...
    # ══════════════════════════════════════════════════════════
    print(f"\n{'='*90}")
    print(f"TRAILING STOP STUDY — QQQ OVERNIGHT (LONG ONLY)")
    print(f"{'='*90}")
    ...
    filters_results.append({'name': 'QQQ > SMA(50)', 'key': 'sma50',
    ...
    filters_results.append({'name': 'QQQ > SMA(50) + SMA(200)', 'key': 'sma50_200',
                           **filter_stats(f_both), 'trades_list': f_both})
    
    # SMA50 + pause after 2
    f_sma50_pause = apply_pause_filter(f_sma50, 2)
    filters_results.append({'name': 'QQQ > SMA(50) + pause after 2 losses', 'key': 'sma50_pause2',
    ...
    filters_results.append({'name': 'QQQ > SMA(50) + no overlap', 'key': 'sma50_nooverlap',
